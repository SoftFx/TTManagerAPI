...
namespace rTTManApi
{
    public class rTTManApiHost
    {
        ...
        public static string[] GetSecurities()
        {
            ...
            var result = groupNames.Select(@group => securities.Where(security => @group.Equals(security.Group)).Aggregate("", (current, security) => string.Concat(current, " ", security.Security))).ToArray();
            return result;
        }
